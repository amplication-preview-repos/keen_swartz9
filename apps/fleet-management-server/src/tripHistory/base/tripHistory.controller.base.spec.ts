...
const service = {
  ...
  tripHistories: () => FIND_MANY_RESULT,
  tripHistory: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("TripHistory", () => {
  ...
  test("GET /tripHistories/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tripHistories"}/${nonExistingId}`)
  ...
  test("GET /tripHistories/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/tripHistories"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /tripHistories existing resource", async () => {
